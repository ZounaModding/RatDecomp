...
void CameraEngine_Z::UpdateFX(Float i_DeltaTime) {
    ...
    Vec3f l_LocalPos = l_NearPos / l_NearPos.GetNorm();
    const Vec3f& l_Plane = l_Camera->GetFrustrum().m_ViewPlanes.m_PlaneNormals[2].xyz();
    l_LocalPos += l_Plane * (1.0f / (l_LocalPos * l_Plane));

    Vec3f l_Position = l_CameraNode->GetWorldMatrix() * l_LocalPos;
    l_Position.y += 2.0f;

    Vec3f l_FirstAxis(0.0f, 0.0f, 1.0f);
    Quat l_FirstRotation(Pi, l_FirstAxis);

    Vec3f l_Direction = l_CameraNode->GetWorldTranslation() - l_Position;
    if (l_Direction.CNormalize()) {
        Vec3f l_SecondAxis(0.0f, -1.0f, 0.0f);
        Quat l_SecondRotation(O_Atan2(l_Direction.x, l_Direction.z), l_SecondAxis);

        Quat l_Rotation;
        l_Rotation = l_FirstRotation * l_SecondRotation;

        for (S32 i = 0; i < l_ParticleAgentNb; i++) {
            if (l_ParticleAgents[i]) {
                l_ParticleAgents[i]->SetPosAndRot(l_Position, l_Rotation);
            }
        }
    }
}
